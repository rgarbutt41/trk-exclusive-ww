...
void  plot_Same_Flavor_w_error(std::string InclWW_Ratio_File, std::string InclWW_Error_File, std::string Zmumu_Ratio_File,std::string Zmumu_Error_File, std::string Ztautau_Ratio_File, std::string Ztautau_Error_File, std::string DYmumu_Ratio_File,std::string DYmumu_Error_File)
{
  ...
  for (int k = 0; k<n; k++)
    {
      ...
      std::cout << Ratio_e[k] << " " << Ratio_sys_e[k] << " " << sqrt_Ratio_e[k] << " " << sqrt_Ratio_sys_e[k] <<"\n";
    }
  ...
  ratio_plot->SetTitle("Same Flavor S/B and S/#sqrt{B} for 1mm PU window (#mu#mu)");
  ratio_plot->GetXaxis()->SetTitle("Min p_{T}(MeV)");
  ratio_plot->GetYaxis()->SetTitle("Respective Ratio of Excl to Inclusive yields");
  ...
  ratio_plot->GetXaxis()->SetRangeUser(50,550);
  ratio_plot->GetYaxis()->SetRangeUser(0,13);
  ratio_plot->Draw("APE");
  ...
  legend->AddEntry(ratio_plot,"S/B");
  ...
  //mg2->SetTitle("Same Flavor S/#sqrt{B} 1mm PU window (mumu)");
  ...
  sqrt_ratio_plot->Draw("PE same");
  sqrt_ratio_plot_sys->Draw("[] same");
  //sqrt_ratio_plot->Draw("AP");
  //mg->Add(sqrt_ratio_plot,"AP");
  //mg->Add(sqrt_ratio_plot_sys,"[]");

  legend->AddEntry(sqrt_ratio_plot,"S/#sqrt{B}");
